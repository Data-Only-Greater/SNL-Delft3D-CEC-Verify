...
def main():
    ...
    dt_init = dt_init_all[:max_experiments]
    ...
    case_counter = 0
    results_dir = Path("structured")
    
    run_directory = results_dir / "grid_convergence_runs"
    run_directory.mkdir(exist_ok=True, parents=True)
    
    report = Report(79, "%d %B %Y")
    report_dir = results_dir / "grid_convergence_report"
